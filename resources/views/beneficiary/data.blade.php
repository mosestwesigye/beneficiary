...
@section('title')
    {{trans_choice('general.beneficiary',2)}}
@endsection
@section('content')
            ...
            <div class="table-responsive">
                <table id="data-table" class="table table-bordered table-condensed table-hover">
                    ...
                    <tr style="background-color: #D1F9FF">
                        ...
                        <th>{{trans_choice('general.national_id',1)}}#</th>
                        <th>{{trans_choice('general.ratio_id',1)}}#</th>
                        <th>{{trans_choice('general.country_of_origin',1)}}#</th>
                        <th>{{trans_choice('general.district_of_residence',1)}}#</th>
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right" role="menu">
                                        @if($key->active==0)
                                            @if(Sentinel::hasAccess('beneficiaries.approve'))
                                                <li><a href="{{ url('beneficiary/'.$key->id.'/approve') }}"><i
                                        ...
                                        @if($key->active==1)
                                            @if(Sentinel::hasAccess('beneficiaries.approve'))
                                                <li><a href="{{ url('beneficiary/'.$key->id.'/decline') }}"><i
                                        ...
                                        @if(Sentinel::hasAccess('beneficiaries.blacklist'))
                                            @if($key->blacklisted==1)
                                                <li><a href="{{ url('beneficiary/'.$key->id.'/unblacklist') }}"
                                                       class="delete"><i
                                            ...
                                            @if($key->blacklisted==0)
                                                <li>
                                                    <a href="{{ url('beneficiary/'.$key->id.'/blacklist') }}"
                                                       class="delete"><i
